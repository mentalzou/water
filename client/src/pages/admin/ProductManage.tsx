...
export default function ProductManage() {
  ...
  const [form, setForm] = useState({ name: '', description: '', price: '', unit: '瓶', brand_id: '' });
  const [editId, setEditId] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [brandFilter, setBrandFilter] = useState('');
  const [keyword, setKeyword] = useState('');

  useEffect(() => { loadBrands(); loadProducts(); }, []);
  useEffect(() => { loadProducts(); }, [brandFilter, keyword]);

  async function loadBrands() {
    try {
      const token = getToken();
      const res: any = await fetch(`${API_BASE}/admin/brands/select`, {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const token = getToken();
      const data = { name: form.name, description: form.description, price: parseFloat(form.price), unit: form.unit, brand_id: form.brand_id || undefined };
  ...
  function openCreate() {
    setForm({ name: '', description: '', price: '', unit: '瓶', brand_id: '' });
    setEditId(null);
    setShowForm(true);
  }
  ...
  function openEdit(p: Product) {
    setEditId(p.id);
    setForm({ name: p.name, description: p.description || '', price: String(p.price), unit: p.unit, brand_id: p.brand_id || '' });
    setShowForm(true);
  }
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { setKeyword(''); setBrandFilter(''); }}
