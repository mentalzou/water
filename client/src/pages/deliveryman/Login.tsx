...
export default function DeliverymanLogin() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    setErrorMsg('');
    try {
      const res = await fetch(`${API_BASE}/deliverymen/login`, {
        method: 'POST',
      ...
      if (data.code === 200) {
        localStorage.setItem('deliveryman_token', data.data.token);
        localStorage.setItem('deliveryman_user', JSON.stringify(data.data));
        navigate('/deliveryman', { replace: true });
      } else {
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          >
            ...
            ) : (<>登录派送中心 <ArrowRight className="w-4 h-4" /></>)}
          </button>

          <div className="flex items-center gap-3 pt-2">
            <div className="h-px flex-1 bg-white/20" />
            <span className="text-white/50 text-xs flex items-center gap-1"><UserCheck className="w-3.5 h-3.5" />派送员专属</span>
            <div className="h-px flex-1 bg-white/20" />
          </div>
